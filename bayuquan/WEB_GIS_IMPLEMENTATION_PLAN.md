...
WebGL 求解器只能作为未来可选的交互式粗略预览，用于方案草绘或教育演示，不得
替代 ANUGA 正式计算。若实施，必须：

- 明确标注“快速预览，非正式计算结果”；
- 与正式 Job 使用不同的数据类型、视觉标识和下载路径；
- 给出分辨率、边界条件、守恒误差和设备差异说明；
- 禁止把预览结果写入正式 Job 或风险报告；
- 使用 WebGPU 或维护中的计算框架重新设计，不直接移植旧 GLOW.js/WebGL 代码。
