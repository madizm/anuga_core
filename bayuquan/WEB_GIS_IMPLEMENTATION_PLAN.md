...
- 动态模式保持 262,144 个稠密 Cell 上限和最高 15 Hz 完整快照；
- 静态快照模式支持 1,048,576 个稠密 Cell，以 1,800 秒精确模拟时间边界更新地图；
- GPU 多级最大波速归约只回读单个 float，CFL 更新不再依赖完整状态回读；
- 静态模式关闭水波计时器、帧间淡入和流向粒子，并将 React 进度发布限制为最高 4 Hz；
- 1M 模式启动前执行 1024×1024 浮点状态纹理试分配，初始化失败时主动释放私有 WebGL 上下文；
- 完整快照复用双缓冲矢量/半浮点数组，并移除未消费的独立水深数组；
- 提供可重复的 1M Cell Chrome/WebGL2 性能基准脚本和基线报告；
