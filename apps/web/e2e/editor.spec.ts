...
test('user locks a local domain before selecting inlet cells', async ({ page }) => {
  ...
  await expect(page.getByText(/网格 5 m · 原始信息 5 m · 原始/)).toBeVisible()
  ...
  const contourLayer = page.locator('.layer-row').filter({ hasText: '等高线与标注' })
  await expect(buildingLayer.locator('input')).toBeDisabled()
  await expect(manningLayer.locator('input')).toBeDisabled()
  await expect(contourLayer.locator('input')).toBeDisabled()
  ...
  await expect(contourLayer.locator('input')).toBeEnabled()
  await expect(page.locator('.model-map')).toHaveAttribute('data-contours-visible', 'true')
  await contourLayer.locator('input').uncheck()
  await expect(page.locator('.model-map')).toHaveAttribute('data-contours-visible', 'false')
  await contourLayer.locator('input').check()
...
test('local-domain COG frames stream into the live playback console', async ({ page }) => {
  ...
  // The water surface is on by default and renders on all three panes.
  await expect(page.locator('.result-map-canvas[data-flow-frame="2"]')).toHaveCount(3)
  await expect(page.getByText('DYNAMIC FLOW')).toHaveCount(3)
  await expect(page.locator('.result-map-canvas[data-water-renderer="terrain-draped"]')).toHaveCount(3)
  // Toggling it off falls back to the raster tile views.
  await page.getByRole('button', { name: /动态水面/ }).click()
  await expect(page.locator('.result-map-canvas[data-flow-frame]')).toHaveCount(0)
  await expect(page.getByText('DYNAMIC FLOW')).toHaveCount(0)
  await page.goto('about:blank')
})
...
function flowPayloadV3() {
  const buffer = Buffer.alloc(44 + 4 * 2)
  buffer.write('BQFV', 0, 'ascii')
  ...
  buffer.writeDoubleLE(122.14, 28)
  buffer.writeDoubleLE(40.25, 36)
  // fp16 texels (u, v, depth, stage): 1.0, 0.5, 0.3, 6.0.
  buffer.writeUInt16LE(0x3c00, 44)
  buffer.writeUInt16LE(0x3800, 46)
  buffer.writeUInt16LE(0x3555, 48)
  buffer.writeUInt16LE(0x4600, 50)
  return buffer
}
...
test('dynamic water toggle mounts the ripple layer without errors', async ({ page }) => {
  ...
  await page.route('**/api/dem-products', (route) => route.fulfill({
    json: { defaultDemProductId: '', products: [] },
  }))
  ...
  await expect(page.getByText('已完成')).toBeVisible()
  // The water surface is the primary view and starts enabled.
...
test('job deep link fits result tiles to its simulation area', async ({ page }) => {
  ...
  await page.goto(`/?job=${jobId}`)
  await expect(page.getByText('已完成')).toBeVisible()
  // Water-primary: particles flow from the binary field without any PNG
  // tile traffic.
  ...
  // Toggling the water view off falls back to the raster tiles.
  await page.getByRole('button', { name: /动态水面/ }).click()
...
function previewTilePayload(fieldCode: number, values: number[] = []) {
  const activeCount = 4
  const payloadBytes = fieldCode === 0 ? 1 : activeCount * 4
  const buffer = Buffer.alloc(32 + payloadBytes)
  buffer.write('BQGT', 0, 'ascii')
  buffer.writeUInt16LE(1, 4)
  buffer.writeUInt16LE(32, 6)
  buffer.writeUInt32LE(fieldCode, 8)
  buffer.writeUInt32LE(0, 12)
  buffer.writeUInt32LE(0, 16)
  buffer.writeUInt32LE(2, 20)
  buffer.writeUInt32LE(2, 24)
  buffer.writeUInt32LE(activeCount, 28)
  if (fieldCode === 0) buffer.writeUInt8(0x0f, 32)
  else values.forEach((value, index) => buffer.writeFloatLE(value, 32 + index * 4))
  return buffer
}
function previewGridManifest() {
  return {
    version: 1, tileSize: 256, demRows: 2, demColumns: 2,
    rowStart: 0, rowStop: 2, columnStart: 0, columnStop: 2,
    corners: [[122, 40.02], [122.02, 40.02], [122, 40], [122.02, 40]],
    fields: ['elevation', 'buildingFraction', 'manningLow', 'manningMiddle', 'manningHigh'],
    tiles: [{
      id: 'r00000-c00000', demColumns: 2,
      rowStart: 0, rowStop: 2, columnStart: 0, columnStop: 2, cellCount: 4,
      topologyUrl: '/api/dem-products/dem-test/simulation-areas/a/grid/tiles/r00000-c00000/topology',
      fieldUrl: '/api/dem-products/dem-test/simulation-areas/a/grid/tiles/r00000-c00000/{field}',
    }],
  }
}
...
test('browser GPU preview follows draft controls and invalidates on scenario edits', async ({ page }) => {
  ...
  await page.route('**/api/dem-products/**', async (route) => {
    ...
    if (url.includes('/grid/')) gridRequests.push(url)
    if (url.endsWith('/simulation-areas/resolve')) {
      return route.fulfill({ json: {
        ...
        gridManifestUrl: '/api/dem-products/dem-test/simulation-areas/a/grid/manifest',
        boundaryCondition: 'transmissive',
      } })
    }
    if (url.endsWith('/grid/manifest')) return route.fulfill({ json: previewGridManifest() })
    const tileField = url.match(/\/grid\/tiles\/[^/]+\/(topology|elevation|buildingFraction|manningLow|manningMiddle|manningHigh)$/)?.[1]
    if (tileField) {
      const codes: Record<string, number> = {
        topology: 0, elevation: 1, buildingFraction: 2,
        manningLow: 3, manningMiddle: 4, manningHigh: 5,
      }
      const values: Record<string, number[]> = {
        elevation: [10, 10, 10, 10], buildingFraction: [0, 0, 0, 0],
        manningLow: [0.03, 0.03, 0.03, 0.03],
        manningMiddle: [0.05, 0.05, 0.05, 0.05],
        manningHigh: [0.1, 0.1, 0.1, 0.1],
      }
      return route.fulfill({ body: previewTilePayload(codes[tileField], values[tileField]), contentType: 'application/octet-stream' })
    }
  ...
  await expect(page.getByText('堤防 1', { exact: true })).toBeVisible()
  await page.locator('.scenario-rail label').filter({ hasText: '模拟时长' }).locator('input').fill('3600')
  ...
  await expect(page.getByText('水工要素采用栅格近似')).toBeVisible()
  ...
  expect(gridRequests.some((url) => url.endsWith('/grid/manifest'))).toBe(true)
  expect(gridRequests.some((url) => url.endsWith('/topology'))).toBe(true)
  expect(gridRequests.some((url) => url.endsWith('/elevation'))).toBe(true)
  expect(gridRequests.every((url) => !url.endsWith('/grid'))).toBe(true)
})
